...
export async function testIngestDrills(): Promise<void> {
  ...
  if (count > 2) {
    await cleanupAndExit(`obvious_single_drill exceeded max drill limit of 2 with ${count} occurrences.`);
  } else if (count < 2) {
    await cleanupAndExit(`Only ${count} obvious_single_drill occurrences were found vs expected 2.`);
  }
  ...
  if (count !== 5) {
    await cleanupAndExit(`Found ${count} drills for first puzzle vs expected 5.`);
  }
  ...
  if (count > 0) {
    await cleanupAndExit(`Found ${count} pointing_triplet_drills vs expected 0.`);
  }
